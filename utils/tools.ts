export function capitalizeFirstWord(str:string) {  
    if (!str) return str; // Check for empty or undefined strings  
...
export function formatTime2(dateString: string | Date | null): string {
  if (!dateString) return "";
  const date = new Date(dateString);
  let hours = date.getHours();
  const minutes = date.getMinutes();
  // console.log('date is in get time',date)
