...
import {projectFormSchema, ProjectFormValues} from "@/utils/supabase/types"
import { DelProjects, GetProjects, GetViewProjects } from "@/app/api/projects/route";
...
function ChangingProjects(value:boolean){
 ...
 useEffect(()=>{
   async function getProjects() {
              ...
              const getData=await GetViewProjects();
              if(getData?.success){
  ...
  const onProjectSelect=(prjId:string)=>{
   ...
   async function deleteProject(prjId:string){
          ...
          setDeleting(true);
          const deleteWorkSpace=await DelProjects(prjId);
  ...
  const SetDialogContent=({isNew}:{isNew:boolean})=>{
    return (
        ...
        <DialogHeader>
          ...
          <ProjectForm onSuccess={() => setIsCreateDialogOpen(false)} onChangedData={ChangingProjects} />
        </DialogContent> :
...
<DialogHeader>
    ...
    <ProjectForm initialData={isSelected} onSuccess={() => setIsCreateDialogOpen(false)} onChangedData={ChangingProjects} />
</DialogContent>
  }
        </>
    )
  ...
  return (
    <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
      {/* <div className="border border-stone-500 w-8 h-8">
          ...
          <TabsContent value={activeTab}>
            {isLoading ? (
              ...
              <Table>
                ...
                <TableBody>
                  {getFilteredTasks(activeTab).map((task: ProjectFormValues) => (
                    <TableRow 
                      key={task.id} 
                    ...
                    >
                      ...
                      {task.clientData ? (
                          <div>
                            ...
                            {task.clientData?.map((c:any)=>(
                                 ...
                                 <Avatar key={`ava-${c.id}`} className=" h-8 w-8 !border border-stone-100/2 shadow-lg ml-[-10px]">
                           ...
                           {task.clientData?.map((c:any,index:any)=>(
                            <div key={`name-${c.id}`} className="text-xs text-muted-foreground">
                              {c.name||''} 
                              {index <(task.clientData?.length ||1)-1  ?<span> ,</span>:'' }
                      ...
                      <TableCell>
                        {task.tasks ? (
                            ...
                            {task.tasks?.map((c:any)=>(
                              
                                
                              <Badge  key={`task-${c.status}`} className={`${c.status === TaskStatusEnum.COMPLETED ? 'bg-green-600': c.status === TaskStatusEnum.CANCELLED ?'bg-pink-600':''}`} title={
                                c.status === TaskStatusEnum.IN_PROGRESS ? "In_Progress" : 
                                c.status === TaskStatusEnum.TODO ? "Todo" :  c.status === TaskStatusEnum.COMPLETED ? "Completed" :
                                "Canceled"} 
                                variant={
                                c.status === TaskStatusEnum.IN_PROGRESS ? "destructive" : 
                                c.status === TaskStatusEnum.TODO ? "default" :  c.status === TaskStatusEnum.COMPLETED ? "" :
                            ...
                            <span>/</span>
                           <span>{task.tasks?.reduce((acc:any, item:any) =>   acc + item.count , 0)}</span>
